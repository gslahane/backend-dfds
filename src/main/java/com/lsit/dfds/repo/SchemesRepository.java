...
//	List<Schemes> findByDistrictAndPlanType(District district, PlanType planType);
	...
	// When district is NOT provided: filter directly on Schemes
	@Query("""
			    select distinct s
			    from Schemes s
			    left join s.financialYear fy
			    left join s.schemeType st
			    where (:financialYearId is null or fy.id = :financialYearId)
			      and (:planType is null or s.planType = :planType)
			      and (:schemeTypeId is null or st.id = :schemeTypeId)
			    order by s.schemeName asc
			""")
	List<Schemes> findFilteredSchemesNoDistrict(@Param("financialYearId") Long financialYearId,
			@Param("planType") PlanType planType, @Param("schemeTypeId") Long schemeTypeId);

	// When district IS provided: go via District.schemes to keep the model
	// unidirectional
	@Query("""
			    select distinct s
			    from District d
			    ...
			    order by s.schemeName asc
			""")
	List<Schemes> findFilteredSchemesForDistrict(@Param("districtId") Long districtId,
			@Param("financialYearId") Long financialYearId, @Param("planType") PlanType planType,
			@Param("schemeTypeId") Long schemeTypeId);

	Optional<Schemes> findFirstByPlanTypeOrderByIdAsc(PlanType mla);

	Optional<Schemes> findByPlanTypeAndFinancialYearId(PlanType hadp, Long fyId);
